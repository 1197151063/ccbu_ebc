...
public class StudentAuthenticationInterceptor extends HandlerInterceptorAdapter {
    ...
    public boolean preHandle(HttpServletRequest request, HttpServletResponse response, Object handler) throws Exception {
        ...
        if (CommonConst.CommonCollection.contain(CommonConst.CommonCollection.IGNORE_URL_WEB, target)) {
            return true;
        }
        //根据session进行校验
        HttpSession session = request.getSession();
        LoginDetailVO employ = (LoginDetailVO)session.getAttribute("app_user_login");
        if(employ == null){
