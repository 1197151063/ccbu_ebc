...
public class OperationService {
    ...
    @Autowired
    private RankingStockService rankingStockService;

    //所有业务提交完成后数据计算
    @Async
    public void depositLoan(Long currentProjectId, Long currentStageId) {
...
//            Double zcx = ((double) 1 / TeamSize) + (1 / mlongLoanCreditRise + 1 / zcl / TeamSize) / cx;
            ...
            map.put("短期贷款", sum * 100);
            map.put("短期贷款-抵押贷款", d * 100);
            map.put("短期贷款-质押贷款", z * 100);
            map.put("短期贷款-保证贷款", b * 100);
            map.put("短期贷款-信用贷款", x * 100);
            map.put("贴现", discountShare * 100);
            map.put("中长期贷款", zcSum * 100);
            map.put("长期贷款-抵押贷款", zcd * 100);
            map.put("长期贷款-质押贷款", zcz * 100);
            map.put("长期贷款-保证贷款", zcb * 100);
            map.put("长期贷款-信用贷款", zcx * 100);
            map.put("短期存款", shortDepositRiseShare * 100);
            map.put("短期储蓄存款", shortSavingsDepositRiseShare * 100);
            map.put("同业存放款项", interbankdepositnnnRiseShare * 100);
            map.put("长期存款", longDePositRiseShare * 100);
            map.put("长期储蓄存款", longDepositRiseShare * 100);
            map.put("委托资产托管业务", custodyBusinessProcedureRiseShare * 100);
            map.put("担保业务", guaranteeBusinessProcedureRiseShare * 100);
...
//            }
            if (decInvestmentLongVO == null) {
                repInvestbusOperatDataStockEntityLong2.setStockCost(0);
                repInvestbusOperatDataStockEntityLong2.setStockNumber(0);
            } else {
                repInvestbusOperatDataStockEntityLong2.setStockCost(decInvestmentLongVO.getBuySell());
                repInvestbusOperatDataStockEntityLong2.setStockNumber((int) Math.round(((long) decInvestmentLongVO.getBuySell() * 1000000000 / shareIndex * 0.001)));
            }
...
////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
            ...
            for (ParBondParameterVO parBondParameterVO : parBondParameterVOS) {
                ...
                //账面损益=（当前阶段的这种债券的价格-上阶段的这种债券的价格）*0.01*上阶段的名义价值
...
//                    repInvestbusOperatDataBondEntity.setAccountProfitLoss(0);
            ...
            List<RepInvestbusOperatDataBondVO> repInvestbusOperatDataBondVOS = repInvestbusOperatDataBondDao.selectInvestbusOperatDataBondRep(currentTeamId, currentStageId, "短期");
            Integer realValue = 0;
            Integer nominalValue = 0;
            Integer interest = 0;
            Integer accountProfitLoss = 0;
            ...
            repInvestbusOperatDataBondEntityNum.setRealValue(realValue);
            repInvestbusOperatDataBondEntityNum.setNominalValue(nominalValue);
            repInvestbusOperatDataBondEntityNum.setRateInterest(null);
            repInvestbusOperatDataBondEntityNum.setInterest(interest);
            repInvestbusOperatDataBondEntityNum.setAccountProfitLoss(accountProfitLoss);
...
//                if (repInvestbusOperatDataBondVO == null) {
//                    //查询参数表0阶段名义价值
//                    ParInitialDataInvestbusBondVO parInitialDataInvestbusBondVO = parInitialDataInvestbusBondDao.selectInvestbusOperatBondData(1L, bondKind, bondType);
//                    if (parInitialDataInvestbusBondVO == null) {
//                        repInvestbusOperatDataBondLongEntity.setNominalValue(decInvestmentLongSharesVO.getBuySell());
//                    } else {
//                        repInvestbusOperatDataBondLongEntity.setNominalValue(parInitialDataInvestbusBondVO.getNominalValue());
//                    }
//                } else {
//                    if (repInvestbusOperatDataBondVO.getNominalValue() != null) {
//                        repInvestbusOperatDataBondLongEntity.setNominalValue(repInvestbusOperatDataBondVO.getNominalValue());
//                    } else {
//                        repInvestbusOperatDataBondLongEntity.setNominalValue(decInvestmentLongSharesVO.getBuySell());
//                    }
//                }.indexOf(parBondParameterVO)
            ...
            List<RepInvestbusOperatDataBondVO> repInvestbusOperatDataBondLongVOS = repInvestbusOperatDataBondDao.selectInvestbusOperatDataBondRep(currentTeamId, currentStageId, "长期");
            Integer realValueLong = 0;
            Integer nominalValueLong = 0;
            Integer interestLong = 0;
            Integer accountProfitLossLong = 0;
            for (RepInvestbusOperatDataBondVO repInvestbusOperatDataBondLongVO : repInvestbusOperatDataBondLongVOS) {
                realValueLong = repInvestbusOperatDataBondLongVO.getRealValue() + realValueLong;
                nominalValueLong = repInvestbusOperatDataBondLongVO.getNominalValue() + nominalValueLong;
                interestLong = repInvestbusOperatDataBondLongVO.getInterest() + interestLong;
                accountProfitLossLong = repInvestbusOperatDataBondLongVO.getAccountProfitLoss() + accountProfitLossLong;
            }
            ...
            repInvestbusOperatDataBondLongEntityNum.setRealValue(realValueLong);
            repInvestbusOperatDataBondLongEntityNum.setNominalValue(nominalValueLong);
            repInvestbusOperatDataBondLongEntityNum.setRateInterest(null);
            repInvestbusOperatDataBondLongEntityNum.setInterest(interestLong);
            repInvestbusOperatDataBondLongEntityNum.setAccountProfitLoss(accountProfitLossLong);
            ...
            //短期贷款
            repBalanceSheetEntity.setShortLoan((int) Math.round((shortTermLoan * sum * 0.01)));
            Integer shortLoan = repBalanceSheetEntity.getShortLoan();

            //贴现
            repBalanceSheetEntity.setDiscount((int) Math.round((amount * discountShare * 0.01)));
            Integer discount = repBalanceSheetEntity.getDiscount();

            //中长期贷款
            repBalanceSheetEntity.setMlongLoan((int) Math.round((mlongTermLoan * zcSum * 0.01)));
            Integer mlongLoan = repBalanceSheetEntity.getMlongLoan();

            //利息收入
            Integer interestIncome = (int) Math.round((shortMortgage * d * 0.01 * decLoanDepositVO.getShortMortgageLoanRise() * 0.01 +
                    shortPledgeLoans * z * 0.01 * decLoanDepositVO.getShortHypothecatedLoanRise() * 0.01 +
                    shortGuaranteedLoan * b * 0.01 * decLoanDepositVO.getShortGuaranteeLoanRise() * 0.01 +
                    shortCreditLoan * x * 0.01 * decLoanDepositVO.getShortLoanCreditRise() * 0.01 +
                    discount * decLoanDepositVO.getDiscountRise() * 0.01 +
                    mlongMortgage * zcd * 0.01 * mlongMortgageLoanRise * 0.01 +
                    mlongPledgeLoans * zcz * 0.01 * decLoanDepositVO.getMlongHypothecatedLoanRise() * 0.01 +
                    mlongGuaranteedLoan * zcb * 0.01 * decLoanDepositVO.getMlongGuaranteeLoanRise() * 0.01 +
                    mlongCreditLoan * zcx * 0.01 * decLoanDepositVO.getMlongLoanCreditRise() * 0.01));
...
////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
            ...
            if (currentStageId >= 3) {
                ...
                for (ParWastageRateVO parWastageRateVO : parWastageRateVOS) {
                    ...
                    //人员调配(千小时)-培训时间
                    repPeopleLogisticsEntity.setTrainingTime(decPersonnelVO.getTrainLoans() * 8 * endEmployeeQuantity * 0.001);
                    Double trainingTime = repPeopleLogisticsEntity.getTrainingTime();
                    trainingTimeSum = (double) (trainingTime + trainingTime) / 0.001;
                    //人员调配(千小时)-市场拓展
                    repPeopleLogisticsEntity.setMarketDevelopment(decPersonnelVO.getBusinessDevelopmentLoans() * effectiveTime * 0.01);
                    Double marketDevelopment = repPeopleLogisticsEntity.getMarketDevelopment();

                    //人员调配(千小时)-客户维护
                    repPeopleLogisticsEntity.setCustomerMaintenance(decPersonnelVO.getCustomerCareLoans() * effectiveTime * 0.01);
                    Double customerMaintenance = repPeopleLogisticsEntity.getCustomerMaintenance();

                    //可用营业时间(千小时)=有效营业时间(千小时)-培训时间-市场拓展-客户维护
                    repPeopleLogisticsEntity.setAvailableTime(effectiveTime - trainingTime + marketDevelopment + customerMaintenance);
                    ...
                    for (RepPeopleLogisticsVO repPeopleLogisticsVO1 : repPeopleLogisticsVOs) {
                        if (repPeopleLogisticsVO1.getBusinessType().equals(parWastageRateVO.getBusinessType())) {
                            repPeopleLogisticsEntity.setNeedTime(repPeopleLogisticsVO1.getNeedTime() + (1 + yewuzengliang) + (1 + parIndexNumberVO.getRateOfChange()));
                        }
                    }
                ...
                repPeopleLogisticsOtherEntity.setNeedTime(repPeopleLogisticsVO.getNeedTime() + (1 + yewuzengliang) + (1 + parIndexNumberVO.getRateOfChange()));
                ...
                if (valueOther < 0) {
                    ...
                    if (valueOther > linOther) {
                        //加班(千小时)
                        repPeopleLogisticsOtherEntity.setWorkOvertime(linOther);
                        //临时工(千小时)
                        repPeopleLogisticsOtherEntity.setTemporaryWorker(valueOther - linOther);
                    } else {
                        //加班(千小时)
                        repPeopleLogisticsOtherEntity.setWorkOvertime(valueOther);
                        //临时工(千小时)
                        repPeopleLogisticsOtherEntity.setTemporaryWorker(0.0D);
                    }
                }
...
/////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
            ...
            if (currentStageId >= 5) {
                ...
                if (repAgencyBondVOS.size() == 0) {
                    ...
                    for (ParAgencyBondIssueVO parAgencyBondIssueVO : parAgencyBondIssueVOS) {
                        if ("甲债券".equals(parAgencyBondIssueVO.getTypesBonds())) {
                        ...
                        } else if ("乙债券".equals(parAgencyBondIssueVO.getTypesBonds())) {
                            repAgencyBondEntity.setCurrentTeamId(decMiddleCustodyVO2.getCurrentTeamId());
                            repAgencyBondEntity.setBondType(parAgencyBondIssueVO.getTypesBonds());
                            String bondType = repAgencyBondEntity.getBondType();
                            repAgencyBondEntity.setCouponRate(parAgencyBondIssueVO.getCouponRate());
                            repAgencyBondEntity.setDeadline(parAgencyBondIssueVO.getDeadline());
                            ...
                            //认购额度？？？
                            repAgencyBondEntity.setSubscriptionLimit(parAgencyBondIssueVO.getSubscriptionLimit());
                            repAgencyBondEntity.setUnderwritingPrice(decMiddleCustodyVO2.getSecondUnderwritPriceBond());
                            Integer Discount = parGencyBondDiscountDao.queryDiscount(currentStageId, bondType);
                            repAgencyBondEntity.setDiscount(Discount);
                            //承销金额*1%
                            //underwritingMount*0.01;
                        ...
                        } else if ("丙债券".equals(parAgencyBondIssueVO.getTypesBonds())) {
                            repAgencyBondEntity.setCurrentTeamId(decMiddleCustodyVO3.getCurrentTeamId());
                            repAgencyBondEntity.setBondType(parAgencyBondIssueVO.getTypesBonds());
                            String bondType = repAgencyBondEntity.getBondType();
                            repAgencyBondEntity.setCouponRate(parAgencyBondIssueVO.getCouponRate());
                            repAgencyBondEntity.setDeadline(parAgencyBondIssueVO.getDeadline());
                            ...
                            //认购额度？？？
                            repAgencyBondEntity.setSubscriptionLimit(parAgencyBondIssueVO.getSubscriptionLimit());
                            repAgencyBondEntity.setUnderwritingPrice(decMiddleCustodyVO3.getThirdUnderwritPriceBond());
                            Integer Discount = parGencyBondDiscountDao.queryDiscount(currentStageId, bondType);
                            repAgencyBondEntity.setDiscount(Discount);
                            //承销金额*1%
                            //underwritingMount*0.01;
                            ...
                            //未承销资格,需支付固定成本3千万
                            repProfitEntity.setSubtractNonbusExpend(subtractNonbusExpend - 3);
                        }
                    }
            ...
            if (preNum != null) {
                repProfitEntity.setSubtractNonbusExpend(subtractNonbusExpend - preNum);
            }
            List<RepPeerReportPurchaseVO> repPeerReportPurchaseVOS = repPeerReportPurchaseDao.queryRecord(currentProjectId, currentTeamId, currentStageId);
            //查询同业报告购买记录表
            if (repPeerReportPurchaseVOS.size() > 0) {
                repProfitEntity.setSubtractNonbusExpend(subtractNonbusExpend - repPeerReportPurchaseVOS.size());
            }
...
////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
            ...
            //...记录点，是否取零阶段
            if (decInvestmentLongBondVO.size() == 0 || decInvestmentLongSharesVO.getBuySell() == null) {
...
//////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
        ...
        //计算综合排名
        rankingService.calculateScore(currentProjectId, currentStageId);
        //计算股价
        rankingStockService.calculateStockPrice(currentProjectId, currentStageId);
        //最后一个阶段计算结束后，结束项目阶段表状态
//        if (currentStageId == 6) {
//            sysProjectStageDao.beginStage(currentProjectId, currentStageId,2);
//        }
        //最后一个阶段计算结束后，结束项目状态
        if (currentStageId == 6) {
            sysProjectDao.updateStatus(currentProjectId, 1);
        }
