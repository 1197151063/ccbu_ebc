...
public class DepartmentService {
    ...
    public ResponseDTO<String> addDepartment(DepartmentCreateDTO departmentCreateDTO) {
        ...
        for (SysStageBusinessVO s : sysStageBusinessVOS) {
            ...
            for (int i = 0; i < split.length; i++) {
                sysProjectTeamStageDao.insertProjectTeamStage(departmentCreateDTO.getProjectId(), departmentEntity.getId(), s.getStageId(), split[i], 0);
            }
        }
    ...
    public ResponseDTO<String> updateDepartment(DepartmentUpdateDTO updateDTO) {
        ...
        departmentDao.updateById(departmentEntity);
        if(entity.getProjectId() != updateDTO.getProjectId()){
            sysProjectTeamStageDao.deleteProjectTeamStage(departmentEntity.getProjectId(),departmentEntity.getId());
            List<SysStageBusinessVO> sysStageBusinessVOS = sysStageBusinessDao.queryAllData();
            for (SysStageBusinessVO s : sysStageBusinessVOS) {
                String[] split = s.getBusinessName().split(",");
                for (int i = 0; i < split.length; i++) {
                    sysProjectTeamStageDao.insertProjectTeamStage(departmentEntity.getProjectId(), departmentEntity.getId(), s.getStageId(), split[i], 0);
                }
            }
        }
        return ResponseDTO.succ();
    ...
    @Transactional(rollbackFor = Exception.class)
    public ResponseDTO<String> updateDepartmentInfo(DepartmentVO updateDTO) {
        if (updateDTO.getParentId() == null) {
            return ResponseDTO.wrap(DepartmentResponseCodeConst.PARENT_ID_ERROR);
        }
        DepartmentEntity entity = departmentDao.selectById(updateDTO.getId());
    ...
    public ResponseDTO<DepartmentVO> getDepartmentById(Long departmentId) {
        ...
        SysProjectVO sysProjectVO = sysProjectDao.queryData(departmentVO.getProjectId());
        if (sysProjectVO.getUpdateTime() != null) {
            if (sysProjectVO.getUpdateTime().compareTo(new Date()) > 0) {
    ...
    public ResponseDTO<List<DepartmentVO>> queryDeptListByProId(Long currentProjectId){
        return ResponseDTO.succData(departmentDao.queryDeptListByProId(currentProjectId));
    }
}
